...
timePropagation = 10.0*60.0;
tstep = 30; 

######### outputs
folderouput = "folderoutput_example";
fileoutputname = "example"
...
to = TimerOutput();
enable_timer!(to)
tV3, resPre = ATTITUDE. attitudeprop_quat_girf(civ,date0,params,tstep,tfinal)
...
ftest = open(string(folderouput,"\\osculatingjulia_",fileoutputname,".txt"),"w");
for kk=1:size(tV3)[1]
...
close(ftest);
fper = open(string(folderouput,"\\osculatingjulia_",fileoutputname,"_performance.txt"),"w");
@printf(fper,"%s\n",to) 
close(fper);

fid = open(string(folderouput,"\\myinput_osculatingjulia.txt"),"w");
